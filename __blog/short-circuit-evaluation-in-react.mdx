...
tags: javascript, react, development, tips
preview: "/blog/short-circuit-evaluation-in-react.jpg"
---

<Hero
  src="/blog/short-circuit-evaluation-in-react.jpg"
  alt="Short-circuit Evaluation in React"
/>
...
It's pretty simple–you just have to remember a few rules. React will render anything that is not `null`, `undefined`, or a `Boolean`. So, in this instance, you have a few options.
...
function MyComponent(props) {
  const { headerCopy } = props;
  return <div>{headerCopy ? <em>{headerCopy}</em> : <strong>N/A</strong>}</div>;
}
```
